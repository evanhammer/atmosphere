    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!testExecutablePath && testProvider === constants_1.UNITTEST_PROVIDER) {
            ...
            promise = pythonExecutionServicePromise.then(executionService => {
                return executionService.execObservable(options.args, Object.assign({}, spawnOptions));
            });
        }
        else if (testExecutablePath) {
            const processService = serviceContainer.get(types_1.IProcessService);
            const envVarsService = serviceContainer.get(types_2.IEnvironmentVariablesProvider);
            promise = envVarsService.getEnvironmentVariables(true, options.workspaceFolder).then(executionService => {
                return processService.execObservable(testExecutablePath, options.args, Object.assign({}, spawnOptions));
            });
        }
        else {
            const pythonServiceFactory = serviceContainer.get(types_1.IPythonExecutionFactory);
            pythonExecutionServicePromise = pythonServiceFactory.create(options.workspaceFolder);
            promise = pythonExecutionServicePromise.then(executionService => {
                return executionService.execModuleObservable(moduleName, options.args, Object.assign({}, spawnOptions));
            });
        }
